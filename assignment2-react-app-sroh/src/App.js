...
class App extends Component {
  ...
  render() {
    ...
    let charComponentObjects = <div>No</div> 
    charComponentObjects = (
      <div>
        {this.state.chars.map((char, index) => {
          return <CharComponent key={index} character={char} click={(event) => this.deleteCharacterHandler(event, index)}/>;
        })}
      </div>
    );
    ...
    return (
      <div className="App">
        ...
        <div>
          ...
          />
          <ValidationComponent textLength={this.state.chars.length} />
          {charComponentObjects}
        </div>
      </div>
    );
